    ...
    if(mounted){
      setState(() {
        ...
        document.data() as Map<String, dynamic>)
            .toList();
        if(keyword!='')this.riddles=riddles.where((riddle)=> riddle['title'].contains(keyword)).toList();
        // Future((){
        //   userInfos=[];
    ...
    return Scaffold(
        appBar: AppBar(
          actions: [
            IconButton(
                onPressed: () async{
                  final Keywords = FirebaseFirestore.instance.collection('Keywords');

                  final histories = (await Keywords.orderBy('searchCount',descending: true).get()).docs
                      .map((DocumentSnapshot document) =>
                  document.data() as Map<String, dynamic>)
                      .toList();
                  keyword = await showSearch(context: context, delegate: searchDelegate(Keywords: Keywords,histories: histories),useRootNavigator: true);
                  loadRiddles();
                },
...
class searchDelegate extends SearchDelegate<String> {
  ...
  Widget buildResults(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      close(context, query);
      if(query.isNotEmpty){
        final snapshotFilter = await Keywords.where('word',isEqualTo: query).get();
        ...
        if(docs.length==0){
          ...
          await Keywords.doc(docs[0].id).update({
